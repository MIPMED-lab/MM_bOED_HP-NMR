...
functions{
   ...
   real[] PyruvateHP_NMR_ODEs4(real t, real[] y, real[] p, real[] x_r, int[] x_i){
      ...
      dInd_dt[11] = dInd_dt[3]*nmCl - (XhpHyper/T1_X);
...
transformed data {
    int nParms = 14; // Number of parameters of the model //--------> Introduce number in generation of script
...
parameters {
    ...
    real<lower=-1> kpl;
    real<lower=-1> kbn;
    real<lower=-1, upper=1> kun;
    real<lower=-0.9473684210526315, upper=4.2631578947368425> kbp;
    real<lower=-1, upper=1.105263157894737> kup;
    real<lower=-1> kunE;
    real<lower=-1, upper=2.3333333333333335> kbnE;
    real<lower=-1, upper=4.2631578947368425> ki;
    real<lower=-1> kr;
    // real ScFm;

    real<lower= -1.7281512605042018> NADH;
    real<lower=-2.4523937007874017> NAD;
    real<lower=-2.041903776513192> LDH;
}
...
transformed parameters {
   ...
   for (j in 1:m){
      theta[1,j] = (T1_X*10)+40.92;
      theta[2,j] = 51.23;
      // theta[3,j] = (kin*0.0055)+0.0055;
      theta[3,j] = (kin*0.0025)+0.0025;
      theta[4,j] = (kpl*0.0275)+0.0275;
      theta[5,j] = (kbn*5e-7)+5e-7;
      theta[6,j] = (kun*0.15)+0.15;
      theta[7,j] = (kbp*0.95)+0.95;
      theta[8,j] = (kup*0.95)+0.95;
      theta[9,j] = (kunE*0.1)+0.1;
      theta[10,j] = (kbnE*3e-6)+3e-6;
      theta[11,j] = (ki*0.95)+0.95;
      // theta[12,j] = (kr*0.0029)+0.0029;
      theta[12,j] = (kr*0.0009)+0.0009;
      theta[13,j] = nmCls[1,j];
      theta[14,j] = ScFms[1,j];
      
      
      inits[1] = (NADH*238)+457;
      inits[2] = (NAD*1270)+3146;
      inits[3] = (LDH*0.1933)+0.3947;



      // theta[1,j] = (T1_X*10)+40.92;
      // theta[2,j] = 51.23;
      // theta[3,j] = (kin*0.0155)+0.0155;
      // theta[4,j] = (kpl*0.0275)+0.0275;
      // theta[5,j] = (kbn*0.035)+0.035;
      // theta[6,j] = (kun*0.15)+0.15;

      // theta[7,j] = (kbp*0.95)+0.95;
      // theta[8,j] = (kup*0.95)+0.95;
      // theta[9,j] = (kunE*0.95)+0.95;
      // theta[10,j] = (kbnE*0.0085)+0.0085;
      // theta[11,j] = (ki*0.95)+0.95;
      // theta[12,j] = (kr*0.0029)+0.0029;
      // theta[13,j] = ScFms[1,j];
