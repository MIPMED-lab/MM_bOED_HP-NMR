...
parameters {
    ...
    real<lower=-1> kpl;
    real<lower=-1> kbn;
    real<lower=-1, upper=3.5> kun;
    real<lower=-1, upper=1.11> kbp;
    real<lower=-1, upper=1.11> kup;
    real<lower=-1, upper=1.11> kunE;
    real<lower=-1> kbnE;
    real<lower=-1, upper=1.11> ki;
    real<lower=-1> kr;
    // real ScFm;

    real<lower=-2.5, upper=3.66> NADH;
    real<lower=-2.5, upper=3.75> NAD;
...
transformed parameters {
   ...
   for (j in 1:m){
      ...
      theta[5,j] = (kbn*0.045)+0.045;
      theta[6,j] = (kun*0.45)+0.45;
...
//    ScFm ~ uniform(-2,2);
   ...
   for (j in 1:m){
      ...
      target += -sqrt(-normal_lpdf(Means[1:stsl[1,j],j,1] | yhat[1:stsl[1,j],j,14], Erros[1:stsl[1,j],j,1]));
